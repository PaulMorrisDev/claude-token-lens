...
- **It uses no Claude tokens unless metrics capture is on.** It reads
  files Claude Code has already written; it never calls Claude or any
  other service itself. Metrics capture (see the Glossary) is the one
  opt-in exception: while it's on, Claude spends a small number of
  tokens reading a note and writing a tag inside your own session.
...
assumes) are in [`docs/concepts.md`](docs/concepts.md). Metrics capture
— what it asks Claude to do, the levels, tags, sampling and privacy —
is in [`docs/capture.md`](docs/capture.md).
...
5. asks whether to turn on metrics capture, after warning that it uses
   tokens and showing what each level would have cost you (off unless
   you choose a level; `--capture-level` answers it up front).
...
| `init` | Detect what's already set up, ask (or, non-interactively, derive) a short question set, write `config.toml` and this project's `projects/<slug>.toml`, print the hook/statusline install fragments, run an initial onboarding baseline, show the exact `settings.json` change that connects the snapshot hook (and a statusline when you have none) and make it only after you say yes, offer to register the service to run at logon (`docs/deploy.md`), and — as its last two steps — ask whether to turn on metrics capture, with what each level would have cost you, and whether to add the `/tl-feedback` skill — see [`docs/onboarding.md`](docs/onboarding.md) and [`docs/capture.md`](docs/capture.md) | `--claude-root PATH` (Claude Code's folder holding `settings.json`; default `$CLAUDE_CONFIG_DIR`, else `~/.claude`), `--answers FILE` (JSON file supplying any subset of the answers), `--non-interactive` (derive unanswered questions instead of prompting; derives to *not* installing the service unless `--install-service` is also given), `--no-install` (skip connecting to Claude Code), `--connect` (make the `settings.json` change without asking; it is still shown, and the file backed up first), `--install-service` (register the service without asking), `--no-service` (skip the logon-service step entirely), `--dry-run` (show the `settings.json` change and the logon-service plan without making either; `config.toml` and the baseline are still written), `--repair-hook` (fix a broken SessionStart hook command without asking: a path a single backslash in JSON broke, an interpreter that can't be found such as a missing `py` launcher, or a `%VARIABLE%` Git Bash doesn't expand; it keeps your own Python when it's found and writes the folder out in full; `settings.json` is backed up first), `--capture-level LEVEL` (answer the metrics capture question without asking: `off` or a level; capture uses tokens), `--capture-no-limit` (answer the time-box question that follows a level with "no limit"; default: capture switches itself off after 14 days), `--feedback {on,off}` (answer the `/tl-feedback` question without asking) |
...
| `backtest` | Read-only: every "what if?" estimate the dashboard logged for you, matched to the real change it turned into and judged against the sessions before and after, plus what's still waiting on a match or more data — see [`docs/backtest.md`](docs/backtest.md) | none; judging itself happens where the data already lives (a running `serve`'s watcher tick, or its own `GET /api/backtest` call), so this only reads `<config-dir>/service.db` |
