...
def test_thinking_is_not_offered_on_opus_5_5_where_it_cant_be_turned_down():
    out = goals.draft("thinking", _with_observed_model("claude-opus-5-5"), UNITS)
    assert out["candidates"] == []


def test_thinking_is_not_offered_on_a_fable_model():
    for model_id in ("claude-fable-5-1", "claude-fable-5"):
        out = goals.draft("thinking", _with_observed_model(model_id), UNITS)
        assert out["candidates"] == [], model_id
