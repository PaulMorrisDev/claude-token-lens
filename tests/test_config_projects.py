...
``src/claudeglass/config.py``: :class:`ProjectConfig`,
...
from claudeglass import config as config_module
...
def test_invalid_apply_scope_raises(tmp_path):
    claudeglass_dir = tmp_path / "claudeglass"
    claudeglass_dir.mkdir()
    (claudeglass_dir / "config.toml").write_text('apply_scope = "somewhere-else"\n', encoding="utf-8")
...
def test_load_project_configs_missing_directory_returns_empty(tmp_path):
    assert load_project_configs(config_dir=tmp_path / "claudeglass") == {}
...
def test_save_and_load_project_config_round_trips(tmp_path):
    ...
    path = save_project_config(claudeglass_dir, "my-slug", project)
    assert path == claudeglass_dir / "projects" / "my-slug.toml"
    assert path.is_file()

    loaded = load_project_configs(claudeglass_dir)
...
def test_save_project_config_only_writes_non_none_fields(tmp_path):
    ...
    path = save_project_config(claudeglass_dir, "slug-a", project)
...
def test_load_project_configs_reads_every_file_keyed_by_stem(tmp_path):
    claudeglass_dir = tmp_path / "claudeglass"
    save_project_config(claudeglass_dir, "proj-a", ProjectConfig(kind="work"))
    save_project_config(claudeglass_dir, "proj-b", ProjectConfig(kind="personal"))
...
def test_invalid_project_kind_raises(tmp_path):
    claudeglass_dir = tmp_path / "claudeglass"
    projects_dir = claudeglass_dir / "projects"
    projects_dir.mkdir(parents=True)
    (projects_dir / "bad.toml").write_text('kind = "hobby"\n', encoding="utf-8")
    with pytest.raises(ConfigError):
        load_project_configs(claudeglass_dir)
...
def test_write_config_values_creates_file_from_scratch(tmp_path):
    claudeglass_dir = tmp_path / "claudeglass"
    path = write_config_values(claudeglass_dir, {"billing": "subscription"})
    assert path == claudeglass_dir / "config.toml"
    config = load_config(config_dir=claudeglass_dir)
...
def test_write_config_values_preserves_untouched_top_level_keys(tmp_path):
    claudeglass_dir = tmp_path / "claudeglass"
    write_config_values(claudeglass_dir, {"billing": "subscription", "min_sessions": 10})
    write_config_values(claudeglass_dir, {"min_turns": 500})
...
def test_write_config_values_merges_nested_table_key_by_key(tmp_path):
    claudeglass_dir = tmp_path / "claudeglass"
    write_config_values(claudeglass_dir, {"thresholds": {"a": 1}})
    write_config_values(claudeglass_dir, {"thresholds": {"b": 2}})
...
def test_write_config_values_rejects_an_invalid_merged_value(tmp_path):
    claudeglass_dir = tmp_path / "claudeglass"
    with pytest.raises(ConfigError):
        write_config_values(claudeglass_dir, {"billing": "not-a-real-mode"})
    # Nothing was written -- validation happens before any write.
    assert not (claudeglass_dir / "config.toml").exists()


def test_write_config_values_falls_back_to_dot_new_for_unsupported_shape(tmp_path):
    claudeglass_dir = tmp_path / "claudeglass"
    claudeglass_dir.mkdir(parents=True)
    # Seed an existing config.toml so the fallback path's "leave the
    # existing file untouched" guarantee is actually exercised.
    (claudeglass_dir / "config.toml").write_text('billing = "api"\n', encoding="utf-8")

    path = write_config_values(
        claudeglass_dir, {"thresholds": {"nested": {"too": "deep"}}}
    )
    assert path == claudeglass_dir / "config.toml.new"
    assert path.is_file()
    # The real config.toml is untouched.
    assert (claudeglass_dir / "config.toml").read_text(encoding="utf-8") == 'billing = "api"\n'
...
def test_write_config_values_escapes_control_characters_and_round_trips(tmp_path):
    ...
    write_config_values(claudeglass_dir, {"pricing_path": tricky})

    lines = (claudeglass_dir / "config.toml").read_text(encoding="utf-8").splitlines()
