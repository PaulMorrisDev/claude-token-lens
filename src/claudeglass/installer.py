"""``claudeglass install-service``/``uninstall-service`` (v3
milestone): register ``claudeglass serve`` to start automatically
...
Nothing here duplicates ``scripts/windows/Register-ClaudeGlassTask.ps1``/
``Unregister-ClaudeGlassTask.ps1``/``scripts/systemd/claudeglass.service``
outright -- those remain the hand-run, copy-pasteable path
``docs/deploy.md`` documents; this module is the same idea driven from
``claudeglass init``/``install-service`` instead, and deliberately
...
TASK_NAME = "ClaudeGlass"
SYSTEMD_UNIT_NAME = "claudeglass.service"
LAUNCHD_LABEL = "com.claudeglass"
...
def detect_pyz_path() -> Path | None:
    ...
    claudeglass.pyz ...`` from the archive's own directory) --
...
def _ps_quote(value: str) -> str:
    ...
    the same way ``Register-ClaudeGlassTask.ps1``'s own quoting already
    has to handle.
    """
...
def _resolve_windows_exe(python_exe: str) -> str:
    ...
    window at logon, matching ``Register-ClaudeGlassTask.ps1``), else
    ``python_exe`` itself.
    """
...
def _serve_argv(exe: str, pyz_path: Path | None, serve_args: list[str]) -> list[str]:
    ...
    return [exe, "-m", "claudeglass", *serve_args]
...
def _plan_windows(python_exe: str, serve_args: list[str], pyz_path: Path | None) -> InstallPlan:
    ...
    # Mirrors Register-ClaudeGlassTask.ps1's own cmdlet sequence exactly
    ...
    # thing Unregister-ClaudeGlassTask.ps1 does by hand.
    stop_command = [
        "powershell.exe",
        "-NoProfile",
        "-ExecutionPolicy",
...
#: Mirrors scripts/systemd/claudeglass.service's own hardening
...
# claudeglass serve -- systemd user unit, written by
# `claudeglass install-service` (mirrors
# scripts/systemd/claudeglass.service). See docs/deploy.md.

[Unit]
Description=claudeglass: local token/prompt-cache analytics service
...
def _plan_linux(python_exe: str, serve_args: list[str], pyz_path: Path | None, config_dir: Path) -> InstallPlan:
    ...
    return InstallPlan(
        ...
        probe_command=["systemctl", "--user", "is-enabled", "claudeglass"],
        notes=[
...
def plan_service_install(
    ...
    *,
    ...
    """Build (never run) the plan to register ``claudeglass
...
def _probe_command_for_platform(plat: str) -> list[str]:
    ...
    if plat == "linux":
        return ["systemctl", "--user", "is-enabled", "claudeglass"]
    if plat == "macos":
...
def plan_lines(
    ...
    *,
    ...
    """What ``plan`` writes (or, for an uninstall, removes) and runs, one
    line each, under ``description`` (default: the plan's own)."""
    lines = [f"claudeglass {action}: {description or plan.description}"]
    lines += [f"  will {file_verb}: {path}" for path in files]
    lines += [f"  will run:   {' '.join(command)}" for command in commands]
    lines += [f"  note: {note}" for note in plan.notes]
    return lines


def _print_plan(action: str, plan: InstallPlan, *, commands: list[list[str]], files: list[Path], **kwargs) -> None:
    for line in plan_lines(action, plan, commands=commands, files=files, **kwargs):
        print(line)
...
def uninstall(
    ...
    *,
    ...
    ``Unregister-ClaudeGlassTask.ps1``/``cli.py``'s own ``serve --purge``.
    """
    _print_plan(
        "uninstall-service",
        plan,
        commands=plan.uninstall_commands,
        files=plan.uninstall_files,
        description="Stop the dashboard and remove its logon service",
        file_verb="remove",
    )
