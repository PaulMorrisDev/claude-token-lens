...
def _default_config_dir() -> Path:
    ...
    ``pricing._default_claudeglass_dir`` and ``discovery.projects_root`` —
...
def resolve_config_dir(cli_arg: str | Path | None = None) -> Path:
    """``--config-dir`` wins; else ``$CLAUDE_CONFIG_DIR/claudeglass``; else
    ``~/.claude/claudeglass``.
    """
    if cli_arg:
        return Path(cli_arg)
...
def prune_usage_log(csv_path: str | Path, retention_days: int, now: datetime | None = None) -> int:
    ...
    :func:`~claudeglass.config.prune_capture_log`'s shape: keep the
...
def build_section(rows: list[dict], token_totals_by_window: dict | None = None):
    ...
    if not rows:
        notes.append(
            "No usage-log rows found. Run `claudeglass log-usage` after "
            "pasting a get_usage result, or install the statusline logger "
            "(`claudeglass statusline --print-install-fragment`)."
        )
...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(prog="claudeglass log-usage")
    parser.add_argument("--config-dir", default=None, help="default: ~/.claude/claudeglass")
