...
def capture_section(
    ...
    if suggestion is not None:
        saved = suggestion["session_note_tokens_saved"] + suggestion["subagent_note_tokens_saved"]
        notes.append(
            f"Every metric {catalogue.LEVEL_TITLES[level]} adds over {catalogue.LEVEL_TITLES[suggestion['target']]} "
            f"has enough of its own evidence ({suggestion['dropped_metrics']} metrics) and self-report "
            f"calibration has settled: stepping down would save about {saved} tokens per session or subagent "
            f"start. '{suggestion['command']}' shows what changes; '{suggestion['undo_command']}' undoes it."
        )
    table = Table(
        name="capture_usage",
