...
the same checklist ``claude-token-lens status`` prints.
...
def _header(detection: onboarding.Detection, stdout: IO[str]) -> None:
    stdout.write("\nToken Lens setup\n")
    stdout.write("Everything stays on this computer: Token Lens reads Claude Code's own files and sends nothing anywhere.\n")
    ...
    if count:
        ...
        stdout.write("No Claude Code history yet: Token Lens starts measuring from your next session.\n")
...
def _connect(opts: Options, tools: Tools, health, *, stdin, stdout, notes: list[str]) -> tuple[str, str]:
    ...
    if tools.hook_command is None:
        ...
        return "blocked", (
            f"{label}: not possible, because this Python's path or Token Lens's folder holds a quote, $, backtick, "
            "or is a UNC path. Move the folder somewhere with a plain path, then run 'claude-token-lens init' again."
        )
    if opts.connect:
        return "yes", ""
    if opts.non_interactive:
        notes.append("connect: not given on the command line; left unconnected (pass --connect to connect)")
        return "no", f"{label}: not now. Run 'claude-token-lens init --connect' to connect."
    stdout.write(
        "\nConnect to Claude Code\n"
        "Token Lens adds a hook to Claude Code's settings.json. When a session starts, it records which settings "
        "the session ran with (names, a few safe values and file sizes, never contents), so Token Lens can measure "
        "what your changes did. It runs in the background and adds no tokens.\n"
    )
    if _yes_no("Connect?", True, stdin=stdin, stdout=stdout):
        return "yes", ""
    return "no", f"{label}: not now. Run 'claude-token-lens init --connect' to connect later."
...
def _service(opts: Options, tools: Tools, setup: _Setup, *, stdin, stdout, notes: list[str]) -> str:
    ...
    if not want:
        return f"{label}: not now. Run 'claude-token-lens install-service' any time to add it."
    setup.service = plan
    if repair:
...
def _decide(opts: Options, tools: Tools, detection, health, *, stdin, stdout, now: datetime) -> _Setup | None:
    ...
    setup.review.append(
        "Save your answers in config.toml, in Token Lens's own folder. How you pay: " + _BILLING_REVIEW.get(billing, billing) + "."
    )
    ...
    if setup.touch_settings:
        ...
        if current.is_on or after.is_on or current.coaching_notes_on:
            ...
            if any(commands.get(spec.script) is None for spec in setup.capture_specs):
                ...
                setup.capture_specs = ()
                capture_line = (
                    f"{_TIPS}: its hooks can't be added, because this Python's path or Token Lens's folder holds a "
                    "quote, $, backtick, or is a UNC path. Move the folder somewhere with a plain path, then run "
                    "'claude-token-lens capture connect'."
                )
        ...
        capture_line = f"{_TIPS}: add the hooks it needs later with 'claude-token-lens capture connect'."
...
def _skill(opts: Options, setup: _Setup, before: CaptureConfig, after: CaptureConfig, may_touch: bool) -> str | None:
    ...
    if want and state in ("missing", "outdated"):
        if not may_touch:
            return "The /tl-feedback skill: add it with 'claude-token-lens capture feedback on'."
        setup.skill = "write"
        ...
        if not may_touch:
            return "The /tl-feedback skill: remove it with 'claude-token-lens capture feedback off'."
        setup.skill = "remove"
...
def _baseline(opts: Options, *, stdout: IO[str], now: datetime) -> None:
    ...
    try:
        ...
        stdout.write("stopped. Run 'claude-token-lens baseline' to take it later.\n")
        return
...
def _summary(opts: Options, setup: _Setup, *, changed: bool, stdout: IO[str], now: datetime) -> None:
    ...
    if setup.running:
        ...
        stdout.write(
            f"Next: run 'claude-token-lens serve', then open {installer.DEFAULT_URL} to see where your tokens go.\n"
        )
    ...
    stdout.write("Check your setup any time: claude-token-lens status\n")
...
def run(
    ...
    *,
    ...
    if opts.dry_run:
        _details(setup, opts, stdout)
        stdout.write("\nDry run: nothing was written. Run 'claude-token-lens init' without --dry-run to set up.\n")
        return 0
    if not _confirm(setup, opts, stdin=stdin, stdout=stdout):
        stdout.write("Nothing was changed. Run 'claude-token-lens init' again when you're ready.\n")
        return 0
