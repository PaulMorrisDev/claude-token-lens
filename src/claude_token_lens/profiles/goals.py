...
#: A lower-effort candidate has nothing to show for itself on these
#: models, so it's never drafted for them. Substring match against the
...
def _thinking(draft: _Draft, tables, *, subagents_only: bool) -> None:
    ...
    for row in tables.rows("agents", "topology_effort_by_agent_type"):
        ...
        if share < THINKING_PCT or (subagents_only and agent == TOP):
            continue
        if _thinking_always_on(observed_models.get(agent)):
            continue
        is_top = agent == TOP
        draft.add(
