...
The report is the same one ``claude-token-lens monthly-report --out DIR``
writes (both go through :func:`claude_token_lens.monthly.run_monthly_report`),
...
both of its files (``claude-token-lens-YYYY-MM.md``/``.html``) exist in
...
_LOG_PREFIX = "claude-token-lens serve: monthly report"
...
class MonthlyReportJob:
    ...
    def run_once(self) -> list[Path]:
        ...
        try:
            ...
            root = self.options.projects_root
            project_dirs = discovery.resolve_project_dirs(root, all_projects=True, exclude_projects=exclude)
