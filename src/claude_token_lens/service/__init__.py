"""``claude-token-lens serve``: a local, read-only HTTP service over a
...
- ``schema.py`` — the SQLite table/index definitions and
  :data:`~claude_token_lens.service.schema.SCHEMA_VERSION`.
- ``store.py`` — :class:`~claude_token_lens.service.store.Store`, the
