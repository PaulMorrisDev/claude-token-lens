...
| `api.js` | `fetchJson`, `loadInto`, `postJson`, `withWindow`, `loadReport`, `loadRecommendations` and `loadQuickActions` (each cached per window), `prefetchActions`, the figures-as-of stamp, and the connection state behind "Service unreachable" |
