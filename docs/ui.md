...
- **Native ES modules.** `index.html` loads
  ...
  graph has no cycles: a module that needs to open another tab calls
  `core.js`'s `showTab`, which `app.js` wires to its tab controller, and
  `api.js` redraws the footer through `figures.notify` rather than
  importing `shell.js`. `tests/test_service_static.py` checks every
...
- **No inline secrets, no auth token in the DOM or a cookie.** The
  ...
  keys are `tls:activeTab` (last-selected tab), `tls:window` (chosen
  window), `tls:sort:<table>` (a table's sort), `tls:theme` (`light` or
  `dark`; anything else follows the system) and the capture banner's
  `tls:captureInviteHidden` and `tls:captureNotesHidden`. An older
  `tls:overviewWindow` is still read once, when `tls:window` is unset.
...
16. **Glossary** — the `GLOSSARY` constant in `page-glossary.js`: each term the
...
| File | Holds |
|---|---|
| `app.js` | the entry point: the tab controller (`TAB_ORDER`, `TAB_RENDERERS`, `activateTab`) and the window picker |
| `core.js` | `el`/`clear`, `localStorage` helpers, the shared `state`, `WINDOW_OPTIONS`, `renderedTabs` and the `showTab` hook |
...
| `links.js` | `SECTION_TAB_MAP`, `TAB_TITLES`/`TAB_INTROS`, `tabHeading`, `tabLink` |
| `shell.js` | what is on every tab: the health banner and footer, the capture banner |
...
| `page-actions.js` | Quick actions, Recommendations |
| `page-spend.js` | Sessions (with the session timeline), Savings, Usage |
| `page-cache.js` | Cache, Cache lifetime (TTL) |
| `page-agents.js` | Agents, Context files |
| `page-habits.js` | Work habits |
| `page-setup.js` | Config, Profiles (with impact and backtest) |
| `page-capture.js` | Capture |
...
promotes it to its own section needs no corresponding dashboard change.
