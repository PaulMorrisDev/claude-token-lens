<#
.SYNOPSIS
    Removes the Scheduled Task Register-ClaudeGlassTask.ps1 created, and
    stops any currently-running claudeglass serve process
...
.DESCRIPTION
    ...
    2. Find and stop any process whose command line invokes
       `claudeglass` (via `pythonw`/`python -m claudeglass
...
.PARAMETER TaskName
    Name of the Scheduled Task to remove. Default: ClaudeGlass
    (must match Register-ClaudeGlassTask.ps1's own default/parameter).

.EXAMPLE
    powershell -ExecutionPolicy Bypass -File Unregister-ClaudeGlassTask.ps1
#>
...
Write-Host "claudeglass: unregistering Scheduled Task '$TaskName'"
...
Write-Host "claudeglass: looking for a running serve process"

$stopped = 0
try {
    $processes = Get-CimInstance Win32_Process -Filter "Name = 'python.exe' OR Name = 'pythonw.exe'" -ErrorAction Stop |
        Where-Object { $_.CommandLine -and $_.CommandLine.ToLower().Contains("claudeglass") }
...
if ($stopped -eq 0) {
    Write-Host "  No running claudeglass process found."
}
else {
...
if (-not $taskRemoved -and $stopped -eq 0) {
    Write-Host "Nothing to do -- claudeglass was not registered or running."
}
else {
    Write-Host "Done."
}
